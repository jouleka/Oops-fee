...
async function sendFreePassNotificationEmail(
  ...
  if (!RESEND_API_KEY) {
    console.log(
      "[settle-promises] Resend API key not configured, skipping free pass email",
    );
    return false;
  }
...
— OopsFee`;
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        "[settle-promises] Resend API error (free pass):",
        response.status,
        errorText,
      );
      return false;
    ...
    const result = await response.json();
    console.log("[settle-promises] Free pass notification email sent:", result.id);
    return true;
  } catch (error) {
    console.error("[settle-promises] Free pass email send error:", error);
    return false;
  }
}
 ...
 * Skip all charges, notify user, and notify friends they won't receive money.
 */
async function handleFreePassSettlement(
  promise: Promise,
  supabase: ReturnType<typeof createAdminClient>,
): Promise<SettlementResult> {
  // Get user's profile for push token and name
  ...
  const pushToken = userProfile?.expo_push_token ?? null;
  ...
  await sendPushNotification(pushToken, "🎟️ Free Pass Used", body, {
    promiseId: promise.id,
    type: "free_pass_used",
  });

  // ─────────────────────────────────────────────────────────────
  // FRIEND NOTIFICATION: Let friends know they won't receive money
  // ─────────────────────────────────────────────────────────────
  if (promise.money_destination === "friend") {
    if (promise.friend_user_id) {
      // In-app friend: send push notification
      ...
      if (friendProfile?.expo_push_token) {
        ...
        await sendPushNotification(
          ...
          "🎟️ Free Pass Used",
          friendBody,
          { promiseId: promise.id, type: "free_pass_no_payout", fromUserId: promise.user_id },
        );
        console.log(
          `[settle-promises] Notified in-app friend ${promise.friend_user_id} about free pass`,
        );
      }
      ...
      if (claim?.friend_email) {
        ...
        console.log(
          `[settle-promises] Sent free pass email to external friend ${claim.friend_email}`,
        );
      }
    }
  }
