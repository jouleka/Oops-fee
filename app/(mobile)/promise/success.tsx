    ...
    <View className="w-[360px] h-[480px] rounded-xl overflow-hidden">
      ...
      >
        <View className="flex-1 items-center justify-center gap-md">
          ...
          <Text className="text-display-lg text-success font-rounded">${promise.stake}</Text>
          <Text className="text-h3 text-white text-center italic">I&apos;d &quot;{promise.text}&quot;</Text>
          <View className="w-[60px] h-0.5 bg-success my-md rounded-sm" />
          <Text className="text-h2 text-white font-rounded">AND I DID IT 💪</Text>
          {streak > 1 && (
            <View className="mt-sm">
              <Text className="text-body-semibold text-warning">🔥 {streak} in a row</Text>
            </View>
          )}
          <Text className="text-caption text-text-muted mt-xl tracking-[2px]">OopsFee</Text>
        </View>
      </LinearGradient>
    </View>
  );
...
function PulsingEmoji({ emoji }: { emoji: string }) {
  ...
  useEffect(() => {
    scale.value = withRepeat(
      withSequence(withSpring(1.1, { damping: 4 }), withSpring(1, { damping: 4 })),
      -1,
      true
    );
  }, [scale]);
...
export default function SuccessScreen() {
  ...
  return (
    <View className="flex-1 bg-black" style={{ paddingTop: insets.top, paddingBottom: insets.bottom }}>
      {/* Confetti */}
      {showConfetti && <Confetti count={60} duration={3000} onComplete={() => setShowConfetti(false)} />}

      {/* Content */}
      <View className="flex-1 items-center justify-center px-xl gap-xl">
        {/* Hero */}
        <Animated.View entering={FadeInUp.delay(200).duration(500)} className="items-center gap-md">
          <PulsingEmoji emoji="🎉" />
          <Text className="text-h1 text-white font-rounded text-center">{title}</Text>
          <Text className="text-body text-text-tertiary text-center italic">{subtitle}</Text>
        </Animated.View>
        ...
        {promise.verificationProof && (
          ...
          >
            <View className="w-12 h-12 rounded-md overflow-hidden relative">
              <Image source={{ uri: promise.verificationProof }} className="w-full h-full" resizeMode="cover" />
            ...
            <View className="flex-1 gap-0.5">
              <Text className="text-label text-success text-[10px]">{VERIFICATION_COPY.proofLabel}</Text>
              <Text className="text-body-semibold text-success text-sm">{VERIFICATION_COPY.verifiedBadge}</Text>
            </View>
          </Animated.View>
        )}
      ...
      {/* Actions */}
      <Animated.View entering={FadeInDown.delay(900).duration(400)} className="px-xl pb-lg gap-md">
        <Pressable
          disabled={sharing}
          onPress={handleShare}
        ...
        >
          <Text className="text-body-semibold text-text-secondary">{SUCCESS_COPY.continueButton}</Text>
        </Pressable>
      </Animated.View>

      {/* Footer */}
      <Animated.View entering={FadeIn.delay(1100).duration(400)} className="px-xl pb-lg items-center">
