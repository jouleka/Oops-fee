...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const doFetch = async (retryCount = 0): Promise<void> => {
      ...
      if (error) {
        // Profile doesn't exist yet - DB trigger should create it
        if (error.code === 'PGRST116') {
          // Retry a few times to wait for the trigger to complete
          if (retryCount < 5) {
            console.log(`[Auth] Profile not found, retrying... (${retryCount + 1}/5)`);
            await new Promise((r) => setTimeout(r, 500));
            return doFetch(retryCount + 1);
          }

          // Fallback: create profile if trigger failed (shouldn't happen)
          console.warn('[Auth] Profile still not found after retries, creating fallback...');
          const displayName =
      ...
      updateLastActive();
      // Claim any pending invite (for returning users too)
      if (retryCount === 0) {
        claimPendingInviteToken();
      }
    };
